...
router.post("/user/register", async (req, res) => {
  ...
  if (emailExists) return res.send({errorState: 3, message: "Email already exists in database"});
  if (usernameExists) return res.send({errorState: 3, message: "Username already exists in database"});
  ...
  if (hashPassword == req.body.password)
    return res.send({errorState: 4, message: "Could not hash password. User Not created"});
...
router.post("/user/login", async (req, res) => {
  ...
  const validPassword = await argon.verify(user.password, req.body.password);
...
router.post("/admin/register", async (req, res) => {
    ...
    const usernameExists = await Admin.findOne({ username: req.body.username.toLowerCase()  });
    if (emailExists) return res.send({errorState: 3, message: "Email already exists in database"});
    if (usernameExists) return res.send({errorState: 3, message: "Username already exists in database"});
